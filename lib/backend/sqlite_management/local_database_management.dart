...
import 'package:CareTalk/models/previous_message.dart';
import 'package:CareTalk/models/connection_primary_info.dart';
...
//all columns
  ...
  final String _colRole = "Role"; //new column specialist || person
...
//all columns
  final String _colCallLogName = "username";
...
//getter for taking instance of database
  ...
  Future<Database> initializeDatabase() async {
    ...
    final Directory newDirectory =
        await Directory("$desiredPath/.Databases/").create(); // await Directory(desiredPath + "/.Databases/").create();
    final String path = "${newDirectory.path}/CareTalk_local_storage.db"; // newDirectory.path + "/Spark_local_storage.db";
  ...
  // create table to store important data using username as primary key
  Future<void> createTableToStoreImportantData() async {
    Database db = await database;

    try {
      await db.execute(
          """CREATE TABLE $_importantTableData($_colUsername TEXT PRIMARY KEY, $_colName TEXT, 
          $_colUserMail TEXT, $_colToken TEXT, $_colProfileImagePath TEXT,
           $_colProfileImageUrl TEXT, $_colAbout TEXT, $_colRole TEXT, $_colWallpaper TEXT,
...
//insert or update important data table
  ...
  ////get username for any user from the importantTableData using email
  Future<String?> getNameForAnyUser(String userEmail) async {
    try {
      final Database db = await database;

      List<Map<String, Object?>> result = await db.rawQuery(
          "SELECT $_colName FROM $_importantTableData WHERE $_colUserMail='$userEmail'");
  ...
  Future<List<LatestMessageFromConnection>>
    ...
    try {
      ...
      List<Map<String, Object?>> getNames = await db.rawQuery(
          """SELECT $_colName FROM $_importantTableData WHERE $_colUserMail != "${FirebaseAuth.instance.currentUser!.email.toString()}" """);
          ...
          if (result.isNotEmpty) {
            ...
            lastestMessageFromConnections.add(
                LatestMessageFromConnection.toJson(
                    name: getNames[i].values.first.toString(),
  ...
  Future<bool> createTableForCallLogs() async {
    try {
      ...
      await db.execute(
          """CREATE TABLE $_callLogData($_colCallLogName Text, $_colCallLogUsername Text, $_colCallLogProfilePic TEXT, $_colCallLogDateTime TEXT, $_colCallLogIspicked TEXT, $_colCallLogIsCaller TEXT )""");
      return true;
    } catch (e) {
      print(
  ...
  Future<bool> insertDataInCallLogsTable(
    ...
    try {
      ...
      //_callLogMap[_colCallLogName] = name;
