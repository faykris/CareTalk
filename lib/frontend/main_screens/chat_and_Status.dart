...
import 'package:CareTalk/frontend/call_screens/pickup_screen.dart';
import 'package:CareTalk/frontend/preview/image_preview.dart';
import 'package:CareTalk/frontend/status_screens/story_view.dart';
import 'package:CareTalk/models/call.dart';
import 'package:CareTalk/models/story_model.dart';
import 'package:CareTalk/services/chat_management/chat_screen.dart';
import 'package:CareTalk/frontend/status_screens/status_maker.dart';
import 'package:CareTalk/global_uses/constants.dart';
import 'package:CareTalk/global_uses/enum_generation.dart';
import 'package:CareTalk/models/latest_message_from_connection.dart';
import 'package:CareTalk/services/search_screen.dart';
...
class _ChatAndStatusScreenState extends State<ChatAndStatusScreen> {
  ...
  Future<void> _fetchRTDataFromFirestore() async {
    ...
    print('real time snapshot: $realTimeSnapshot');
    //check for changes
    realTimeSnapshot!.listen((querySnapshot) {
      querySnapshot.docs.map((queryDocumentSnapshot) async {
  ...
  Future<void> _checkingForMessagesAndStatus(
    ...
    try {
      ...
      _connectionRequestList.map((connectionRequestData) {
        if (connectionRequestData.values.first.toString() ==
          ...
          docs.map((everyDocument) async {
            if (everyDocument.id ==
              ...
              print('call: ${everyDocument.get(_firestoreFieldConstants.call)}');

              if (_callData.isNotEmpty) {
                //CHECK FOR NEW CALLS
                Call call = Call.fromMap(_callData); // Unhandled Exception: type 'Null' is not a subtype of type 'String'
                if (call.receiverId ==
                    FirebaseAuth.instance.currentUser!.email.toString()) {
                  Navigator.push(
                      context,
  ...
  void _loadLatestMessageReceivedFromConnection() async {
    try {
      ...
      for (int i = 0; i < _latestMessageFromConnections.length; i++) {
        ...
        if (mounted) {
          setState(() {
            print('latestMessageFromConnection.name: ${latestMessageFromConnection.name}');
            _latestMessages.add(LatestMessageFromConnection(
                name: latestMessageFromConnection.name,
  ...
  void initState() {
    ...
    print('before _fetchRTDataFromFirestore' );
                            ...
                            children: [
                              ...
                              IconButton(
                                  icon: const Icon(Icons.more_vert_outlined,
                                      size: 21.0, color: kSecondaryAppColor),
                                  onPressed: () {}),
              ...
              _latestMessages.isEmpty
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (context, position) {
                        print(_latestMessages);
                        return Column(children: [
                          //   display statuses first - DISABLED
...
//friends' status
  ...
  Widget _chatTile(BuildContext context, int index) {
    ...
    return ListTile(
        ...
        title: Text(
            _latestMessages[index].name.length <= 18
                ? _latestMessages[index].name
                : _latestMessages[index].name.replaceRange(
                    18,
                    _latestMessages[index].name.length,
